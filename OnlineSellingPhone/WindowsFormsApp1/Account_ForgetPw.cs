...
namespace WindowsFormsApp1
{
    public partial class Account_ForgetPw : UserControl
    {
        ...
        private void btnVerifyCode_Click(object sender, EventArgs e)
        {
            if(txtUsernameVerify.Text != "")
            {
                if (txtVerifyCode.Text != "")
                {
                    ...
                    {
                        MessageBox.Show("Mã sai!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    MessageBox.Show("Vui lòng nhập code để xác minh!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
        ...
        private void btnSendEmailVerify_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng không để trống tên đăng nhập!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
