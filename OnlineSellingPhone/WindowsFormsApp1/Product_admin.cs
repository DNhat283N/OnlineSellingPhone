...
{
    public partial class Product_admin : UserControl
    {
        ...
        {
            if (!Form1.Instance.PnlContainer.Controls.ContainsKey("Product_add_admin"))
            {
                Product_add_admin PAdd = new Product_add_admin();
                PAdd.Dock = DockStyle.Fill;
                Form1.Instance.PnlContainer.Controls.Add(PAdd);
            }
            Form1.Instance.PnlContainer.Controls["Product_add_admin"].BringToFront();
        }

        private void dsButton1_Click(object sender, EventArgs e)
        {
            //pnlProduct.Controls.Remove()
        }
    }
}
